...
const std::string SECRET_KEY = adsq("R7mK9pL2nQ8xT6vF4bH3jC1yA5sE0wZu");
...
std::vector<unsigned char> parseByteArray(const std::string& data) {
    ...
    size_t start = data.find(adsq("unsigned char rawData["));
    if (start == std::string::npos) {
        std::cerr << adsq("[ERROR] Start of byte array not found\n");
        return result;
    }

    start = data.find("{", start);
    if (start == std::string::npos) {
        std::cerr << adsq("[ERROR] Opening brace of byte array not found\n");
        return result;
    }
    start++;
    ...
    if (end == std::string::npos) {
        std::cerr << adsq("[ERROR] Closing brace of byte array not found\n");
        return result;
    }
